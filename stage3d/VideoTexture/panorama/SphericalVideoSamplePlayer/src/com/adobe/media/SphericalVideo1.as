package com.adobe.media
{
	...
	import com.media.simpleVideo.VideoCallbackClient1;
	...
	/**
	 * 渲染材质到位图，以查看videoTexture尺寸 
	 * @author disini
	 * 
	 */	
	public class SphericalVideo1 extends EventDispatcher
	{
...
//			fs0为 纹理寄存器，2d 是说明纹理格式是2D的；
		...
		private var callbackClient:VideoCallbackClient1;
...
//		public function SphericalVideo1(target:IEventDispatcher=null)
		public function SphericalVideo1(stage:Stage, x:int, y:int, width:int, height:int):void
		{
//			super(target);
			_stage = stage;
//			stageWidth = width;
...
//			bitmapData.unlock();
		...
		public function setFOV(fovY:Number, zNear:Number, zFar:Number):void
		{
			...
			/*显示效果分析 : 对于4k分辨率的视频，3840x2160, fovY = 50°=>fovX = 80°，
			3840x80/360 = 3840x0.22 = 853； 2160x50/180 = 2160x0.277 = 600, 所以舞台设为960x540的话，
			分辨率接近853x600, 应该不会出现失真才对，但是现在根据renderToTexture的情况；来看，很明显的失真了，
			所以可判断VideoTexture是将视频的分辨率进行了压缩*/
			var fovX:Number = _stage.stageWidth / _stage.stageHeight * fovY;// fovX = 81°
...
//			context3D.configureBackBuffer(_stage.stageWidth*2, _stage.stageHeight*2, 16, true, true, true);
			context3D.configureBackBuffer(_stage.stageWidth*4, _stage.stageHeight*4, 16, true, true, true);
...
//				_bitmap = new Bitmap(new BitmapData(_stage.stageWidth * 2,_stage.stageHeight*2,true,0xFF000000));
				_bitmap = new Bitmap(new BitmapData(_stage.stageWidth * 4, _stage.stageHeight*4, true, 0xFF000000));
//				_bitmap.scaleX = _bitmap.scaleY = 0.5;
				_bitmap.scaleX = _bitmap.scaleY = 0.25;
//				_bitmap.alpha = 0.6;
//				_stage.addChild(_bitmap);
				_stage.addChildAt(_bitmap, 0);
			}
			if(_changeKey)
			{
				_changeKey = false;
//				_bitmap.bitmapData= new BitmapData(_stage.stageWidth * 2, _stage.stageHeight*2, true, 0xFF000000);
				_bitmap.bitmapData= new BitmapData(_stage.stageWidth * 4, _stage.stageHeight*4, true, 0xFF000000);
//				_bitmap.scaleX = _bitmap.scaleY = 0.5;
				_bitmap.scaleX = _bitmap.scaleY = 0.25;
//				_bitmap.bitmapData.width = _stage.stageWidth * 2;
//				_bitmap.bitmapData.height = _stage.stageHeight* 2;
				//_stage.addChild(_bitmap);
			}
...
//			var ns:NetStream; 
//			var callbackClient:VideoCallbackClient1;
			if(!nc)
			...
			if(!callbackClient)
				callbackClient = new VideoCallbackClient1(this);
...
//			context3D.configureBackBuffer(_stage.stageWidth*2, _stage.stageHeight*2, 16, true, true, true);
			context3D.configureBackBuffer(_stage.stageWidth*4, _stage.stageHeight*4, 16, true, true, true);// 缓冲区大小支持7680x4320
		}
